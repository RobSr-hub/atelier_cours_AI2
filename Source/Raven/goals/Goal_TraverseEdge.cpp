...
void Goal_TraverseEdge::Activate()
{
  ...
  switch(m_Edge.Behavior())
  {
    ...
    {
      m_pOwner->SetMaxSpeed(config.GetDouble("Bot_MaxSwimmingSpeed"));
    }
    ...
    {
       m_pOwner->SetMaxSpeed(config.GetDouble("Bot_MaxCrawlingSpeed"));
    }
...
void Goal_TraverseEdge::Terminate()
{
  ...
  m_pOwner->SetMaxSpeed(config.GetDouble("Bot_MaxSpeed"));
}
...
void Goal_TraverseEdge::Render()
{
  ...
  {
    gfx.BluePen();
    gfx.Line(m_pOwner->Pos(), m_Edge.Destination());
    gfx.GreenBrush();
    gfx.BlackPen();
    gfx.Circle(m_Edge.Destination(), 3);
  }
}
