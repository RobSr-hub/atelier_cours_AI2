    ...
    , m_pGraveMarkers(NULL)
{
  ...
  LoadMap(config.GetString("StartMap"));
}
...
bool Raven_Scene::AttemptToAddBot(Raven_Bot* pBot)
{
  ...
  {
    TraceLog(LOG_ERROR, "Map has no spawn points!"); return false;
  }
...
bool Raven_Scene::LoadMap(const std::string& filename)
{
  ...
  //in with the new
  m_pGraveMarkers = new GraveMarkers(config.GetDouble("GraveLifetime"));
  m_pPathManager = new PathManager<Raven_PathPlanner>(config.GetInt("MaxSearchCyclesPerUpdateStep"));
...
void Raven_Scene::ClickRightMouseButton(Vector2D p)
{
  ...
  if (m_pSelectedBot->isPossessed())
  {
    ...
    if (IsKeyDown('Q'))
    {
      m_pSelectedBot->GetBrain()->QueueGoal_MoveToPosition(p);
    }
    else
    {
      //clear any current goals
...
bool
Raven_Scene::isSecondVisibleToFirst(const Raven_Bot* pFirst,
