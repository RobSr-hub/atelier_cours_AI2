...
Raven_Bot::Raven_Bot(Raven_Scene* world,Vector2D pos):
  ...
  MovingEntity(pos,
               config.GetDouble("Bot_Scale"),
               Vector2D(0,0),
               config.GetDouble("Bot_MaxSpeed"),
               Vector2D(1,0),
               config.GetDouble("Bot_Mass"),
               Vector2D(config.GetDouble("Bot_Scale"),config.GetDouble("Bot_Scale")),
               config.GetDouble("Bot_MaxHeadTurnRate"),
               config.GetDouble("Bot_MaxForce")),
                 
                 m_iMaxHealth(config.GetInt("Bot_MaxHealth")),
                 m_iHealth(config.GetInt("Bot_MaxHealth")),
                 m_pPathPlanner(NULL),
                 m_pSteering(NULL),
                 m_pWorld(world),
                 m_pBrain(NULL),
                 m_iNumUpdatesHitPersistant((int)(Game::FPS * config.GetDouble("HitFlashTime"))),
                 m_bHit(false),
                 m_iScore(0),
                 m_Status(spawning),
                 m_bPossessed(false),
                 m_dFieldOfView(DegsToRads(config.GetDouble("Bot_FOV")))
           
{
  ...
  m_pWeaponSelectionRegulator = new Regulator(config.GetDouble("Bot_WeaponSelectionFrequency"));
  m_pGoalArbitrationRegulator =  new Regulator(config.GetDouble("Bot_GoalAppraisalUpdateFreq"));
  m_pTargetSelectionRegulator = new Regulator(config.GetDouble("Bot_TargetingUpdateFreq"));
  m_pTriggerTestRegulator = new Regulator(config.GetDouble("Bot_TriggerUpdateFreq"));
  m_pVisionUpdateRegulator = new Regulator(config.GetDouble("Bot_VisionUpdateFreq"));
  ...
  m_pWeaponSys = new Raven_WeaponSystem(this,
                                        config.GetDouble("Bot_ReactionTime"),
                                        config.GetDouble("Bot_AimAccuracy"),
                                        config.GetDouble("Bot_AimPersistance"));

  m_pSensoryMem = new Raven_SensoryMemory(this, config.GetDouble("Bot_MemorySpan"));
}
...
void Raven_Bot::SetUpVertexBuffer()
{
  ...
  double scale = config.GetDouble("Bot_Scale");
