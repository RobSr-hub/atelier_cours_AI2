...
RocketLauncher::RocketLauncher(Raven_Bot*   owner):
                      ...
                      Raven_Weapon(type_rocket_launcher,
                                   config.GetInt("RocketLauncher_DefaultRounds"),
                                   config.GetInt("RocketLauncher_MaxRoundsCarried"),
                                   config.GetDouble("RocketLauncher_FiringFreq"),
                                   config.GetDouble("RocketLauncher_IdealRange"),
                                   config.GetDouble("Rocket_MaxSpeed"),
                                   owner)
{
...
{ 
  if (NumRoundsRemaining() > 0 && isReadyForNextShot())
  {
    ...
    m_pOwner->GetWorld()->GetMap()->AddSoundTrigger(m_pOwner, config.GetDouble("RocketLauncher_SoundRange"));
  }
}
