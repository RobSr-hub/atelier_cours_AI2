...
Rocket::Rocket(Raven_Bot* shooter, Vector2D target):
        ...
        Raven_Projectile(target,
                         ...
                         shooter->Pos(),
                         shooter->Facing(),
                         config.GetInt("Rocket_Damage"),
                         config.GetDouble("Rocket_Scale"),
                         config.GetDouble("Rocket_MaxSpeed"),
                         config.GetDouble("Rocket_Mass"),
                         config.GetDouble("Rocket_MaxForce")),

       m_dCurrentBlastRadius(0.0),
       m_dBlastRadius(config.GetDouble("Rocket_BlastRadius"))
{
...
void Rocket::Update()
{
  ...
  {
    m_dCurrentBlastRadius += config.GetDouble("Rocket_ExplosionDecayRate");
