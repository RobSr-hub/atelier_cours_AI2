...
RailGun::RailGun(Raven_Bot*   owner):
                      ...
                      Raven_Weapon(type_rail_gun,
                                   config.GetInt("RailGun_DefaultRounds"),
                                   config.GetInt("RailGun_MaxRoundsCarried"),
                                   config.GetDouble("RailGun_FiringFreq"),
                                   config.GetDouble("RailGun_IdealRange"),
                                   config.GetDouble("Slug_MaxSpeed"),
                                   owner)
{
...
{ 
  if (NumRoundsRemaining() > 0 && isReadyForNextShot())
  {
    ...
    m_pOwner->GetWorld()->GetMap()->AddSoundTrigger(m_pOwner, config.GetDouble("RailGun_SoundRange"));
  }
}
