...
ShotGun::ShotGun(Raven_Bot*   owner):
                      ...
                      Raven_Weapon(type_shotgun,
                                   config.GetInt("ShotGun_DefaultRounds"),
                                   config.GetInt("ShotGun_MaxRoundsCarried"),
                                   config.GetDouble("ShotGun_FiringFreq"),
                                   config.GetDouble("ShotGun_IdealRange"),
                                   config.GetDouble("Pellet_MaxSpeed"),
                                   owner),

            m_iNumBallsInShell(config.GetInt("ShotGun_NumBallsInShell")),
            m_dSpread(config.GetDouble("ShotGun_Spread"))
{
...
{ 
  if (NumRoundsRemaining() > 0 && isReadyForNextShot())
  {
    ...
    m_pOwner->GetWorld()->GetMap()->AddSoundTrigger(m_pOwner, config.GetDouble("ShotGun_SoundRange"));
  }
}
